<template>
  <div>
    <div class="title">
      <span style="font-family: 宋体,serif; font-size: 25px; margin-left: 15px">个人相关批次</span>
...
<script>
import {historyListSelect, historyList, userBatch, userBatchSelect} from '@/api/batch'
...
export default {
  ...
  methods : {
    ...
    getBatchList(){
      userBatch(this.userId).then(response => {
    ...
    batchSelect(){
      ...
      userBatchSelect(this.userId, this.batchInfoInput, this.batchTypeSelect, this.batchStatusSelect).then(response => {
        const batches = response.data.batches;
        const promises = batches.map(batch => {
          return Promise.all([
            this.selectNameAndAccountById(batch.operatorId),
